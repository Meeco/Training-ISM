...
// ---------- Props ----------
export interface AwarenessTrainingProps {
  companyName?: string;
...
const cal = (t: "good" | "warn" | "bad" | "info", title: string, text: string): Block => {
  const tone = { good: { accentC: "#1f7a52", tint: "#e7f4ed" }, warn: { accentC: "#b9791d", tint: "#f8efdc" }, bad: { accentC: "#b23a3a", tint: "#fbece9" }, info: { accentC: "#1f4e79", tint: "#eaf0f7" } }[t];
...
function buildScreens(): Screen[] {
  ...
  S.push({
    ...
    blocks: [cards(2, [
      ...
      { tag: "EMPLOYEE INFORMATION", color: "#7a3b6e", title: "About our own people", items: ["Home address & contact details", "Bank, super & payroll details", "Employment & leave records"] },
...
// ---------- Component ----------
export default function AwarenessTraining({ companyName = "[Company Name]", trainingVersion = "v2026.1", passMark = 75, accent = "#1f4e79" }: AwarenessTrainingProps) {
  ...
  const optStyle = (state: string): React.CSSProperties => {
    const base: React.CSSProperties = { display: "flex", gap: 12, alignItems: "center", width: "100%", padding: "13px 15px", border: "1.5px solid #e0e6ec", borderRadius: 12, background: "#fff", cursor: "pointer", fontSize: 14.5, color: "#1f2c3d", fontFamily: "inherit", transition: "all .15s", textAlign: "left" };
    if (state === "selected") return { ...base, borderColor: accent, background: "#f3f7fc" };
  ...
  const badgeStyle = (state: string): React.CSSProperties => {
    ...
    return { ...base, background: "#eef1f5", color: "#7a8696" };
  };
  ...
  const ghost: React.CSSProperties = { padding: "11px 20px", borderRadius: 10, border: "1px solid #d4dae2", background: "#fff", color: "#41506a", fontSize: 13.5, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" };
  const inputStyle: React.CSSProperties = { padding: "11px 13px", borderRadius: 9, border: "1.5px solid #d8dee6", fontSize: 14.5, fontFamily: "inherit", color: "#1f2c3d", background: "#fff", outline: "none", width: "100%" };
  ...
  const renderBlock = (blk: Block, i: number) => {
    if ("isCards" in blk) return (
      <div key={i} style={{ display: "grid", gridTemplateColumns: blk.gridCols, gap: 12 }}>
        {blk.cards.map((c, ci) => (
          <div key={ci} style={{ background: "#fff", border: "1px solid #e4e9ef", borderTop: `4px solid ${c.color}`, borderRadius: 12, padding: "15px 15px 14px" }}>
            <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, letterSpacing: ".12em", fontWeight: 600, color: c.color }}>{c.tag}</div>
            <div style={{ fontSize: 15.5, fontWeight: 600, color: "#16243a", margin: "6px 0 9px", lineHeight: 1.25 }}>{c.title}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {c.items.map((it, ii) => <div key={ii} style={{ fontSize: 12.5, color: "#54627a", lineHeight: 1.4 }}>{it}</div>)}
            </div>
          </div>
        ))}
      </div>
    ...
    if ("isBullets" in blk) return (
      <div key={i}>
        {blk.title && <div style={{ fontSize: 15, fontWeight: 600, color: "#16243a", marginBottom: 11 }}>{blk.title}</div>}
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {blk.items.map((b, bi) => (
            <div key={bi} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div style={{ fontSize: 14.5, lineHeight: 1.45 }}><span style={{ fontWeight: 600, color: "#16243a" }}>{b.h}</span> <span style={{ color: "#54627a" }}>{b.t}</span></div>
            </div>
          ))}
        </div>
      </div>
    ...
    if ("isColumns" in blk) return (
      <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ background: "#fff", border: "1px solid #e4e9ef", borderRadius: 12, padding: "15px 16px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {blk.left.map((li, li_i) => <div key={li_i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}><div style={{ color: blk.leftAccent, fontWeight: 700, fontSize: 13, flex: "none" }}>{blk.leftMark}</div><div style={{ fontSize: 13, color: "#43506a", lineHeight: 1.4 }}>{li}</div></div>)}
          </div>
        </div>
        <div style={{ background: "#fff", border: "1px solid #e4e9ef", borderRadius: 12, padding: "15px 16px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {blk.right.map((ri, ri_i) => <div key={ri_i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}><div style={{ color: blk.rightAccent, fontWeight: 700, fontSize: 13, flex: "none" }}>{blk.rightMark}</div><div style={{ fontSize: 13, color: "#43506a", lineHeight: 1.4 }}>{ri}</div></div>)}
          </div>
        </div>
      </div>
    );
    if ("isCallout" in blk) return (
      <div key={i} style={{ background: blk.tint, borderLeft: `4px solid ${blk.accentC}`, borderRadius: "0 10px 10px 0", padding: "14px 18px" }}>
        {blk.title && <div style={{ fontWeight: 600, color: "#16243a", marginBottom: 4, fontSize: 14.5 }}>{blk.title}</div>}
        <div style={{ color: "#41506a", fontSize: 13.5, lineHeight: 1.5 }}>{blk.text}</div>
      </div>
    );
    if ("isBanner" in blk) return (
      <div key={i} style={{ background: "#16243a", color: "#fff", borderRadius: 12, padding: "16px 20px", fontSize: 15.5, fontWeight: 600, lineHeight: 1.4 }}>{blk.text}</div>
    );
    return null;
  };
  ...
  const renderQuestion = (s: QuestionScreen) => {
    ...
    return (
      <div style={{ marginBottom: 32, maxWidth: 840, background: "#fff", border: "1px solid #e7ebf1", borderRadius: 14, padding: "20px 22px" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#eaf0f7", borderRadius: 20, padding: "5px 13px", marginBottom: 13 }}>
        ...
        {s.context && <div style={{ background: "#f7f9fb", border: "1px solid #e7ebf1", borderRadius: 12, padding: "14px 16px", marginBottom: 14, fontSize: 14, color: "#3a4759", lineHeight: 1.55 }}>{s.context}</div>}
        <h3 style={{ fontSize: 19, fontWeight: 700, color: "#16243a", margin: "0 0 14px", lineHeight: 1.3 }}>{s.prompt}</h3>
        ...
        {answered && (
          <div style={{ background: good ? "#eaf5ee" : "#fbece9", borderRadius: 12, padding: "14px 16px", marginTop: 14, borderLeft: `4px solid ${good ? "#1f7a52" : "#b23a3a"}` }}>
            ...
            <div style={{ fontSize: 13.5, color: "#3a4759", lineHeight: 1.5 }}>{good ? s.explainRight : s.explainWrong}</div>
          </div>
        )}
      </div>
    );
  ...
  const renderPhish = (s: PhishScreen) => {
    ...
    const tagStyle = (f: boolean): React.CSSProperties => ({ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11.5, fontWeight: 600, color: f ? "#b23a3a" : "#3a4759", background: f ? "#fbece9" : "transparent", border: "none", padding: "3px 5px", borderRadius: 5, cursor: "pointer", textAlign: "left", textDecoration: f ? "line-through" : "none" });
    const subjStyle = (f: boolean): React.CSSProperties => ({ display: "block", marginTop: 6, fontSize: 14, fontWeight: 700, color: f ? "#b23a3a" : "#16243a", background: f ? "#fbece9" : "transparent", border: "none", padding: "3px 5px", borderRadius: 5, cursor: "pointer", textAlign: "left", width: "100%" });
    const lineStyle = (f: boolean): React.CSSProperties => ({ fontSize: 13.5, color: f ? "#b23a3a" : "#3a4759", background: f ? "#fbece9" : "transparent", border: "none", padding: "2px 5px", borderRadius: 5, cursor: "pointer", textAlign: "left", fontFamily: "inherit", fontWeight: f ? 600 : 400 });
    ...
    const linkStyle = (f: boolean): React.CSSProperties => ({ fontSize: 13, color: f ? "#b23a3a" : "#2f6fb0", background: f ? "#fbece9" : "#f3f7fc", border: `1px solid ${f ? "#eccac4" : "#dbe7f3"}`, padding: "8px 10px", borderRadius: 8, cursor: "pointer", textAlign: "left", fontFamily: "'IBM Plex Mono',monospace", wordBreak: "break-all" });
    return (
      <div style={{ marginBottom: 32 }}>
        <div style={{ marginBottom: 14 }}>
          ...
          <h3 style={{ fontSize: 22, fontWeight: 700, color: "#16243a", margin: "7px 0 5px" }}>{s.title}</h3>
          <div style={{ color: "#56657a", fontSize: 14, lineHeight: 1.5, maxWidth: 760 }}>{s.intro}</div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1.55fr 1fr", gap: 18, alignItems: "start" }}>
          <div style={{ border: "1px solid #e2e7ee", borderRadius: 12, overflow: "hidden", background: "#fff" }}>
            <div style={{ padding: "13px 16px", borderBottom: "1px solid #eef1f5", background: "#f7f9fb" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <button onClick={() => toggleFlag(s.id, "from")} style={tagStyle(isF("from"))}>{s.from}</button>
                <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, color: "#9aa6b4" }}>{s.time}</div>
              </div>
              <button onClick={() => toggleFlag(s.id, "subj")} style={subjStyle(isF("subj"))}>{s.subject}</button>
            </div>
            <div style={{ padding: "16px 18px", display: "flex", flexDirection: "column", gap: 11, fontSize: 13.5, color: "#3a4759", lineHeight: 1.5 }}>
              ...
              <div style={{ color: "#7a8696", fontSize: 12.5 }}>Regards,<br />Accounts Team</div>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ border: "1px solid #e2e7ee", borderRadius: 12, padding: "15px 16px", background: "#fff" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#16243a" }}>Red flags found</div>
                <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 15, fontWeight: 600, color: accent }}>{found} / 6</div>
              </div>
              <div style={{ height: 6, background: "#eef1f5", borderRadius: 4, marginTop: 9, overflow: "hidden" }}><div style={{ height: "100%", width: pct, background: accent, transition: "width .25s" }} /></div>
              <div style={{ fontSize: 12.5, color: "#56657a", lineHeight: 1.45, marginTop: 11 }}>{hint}</div>
              {!complete && <button onClick={() => revealPhish(s.id)} style={{ marginTop: 12, background: "transparent", border: "1px solid #d4dae2", color: "#56657a", fontSize: 12.5, fontWeight: 600, padding: "8px 14px", borderRadius: 8, cursor: "pointer" }}>Reveal remaining flags</button>}
            </div>
            {complete && (
              <div style={{ background: "#e4f3ec", borderLeft: "4px solid #1f7a52", borderRadius: "0 10px 10px 0", padding: "13px 16px" }}>
                <div style={{ fontWeight: 600, color: "#16243a", fontSize: 13.5, marginBottom: 3 }}>That's all six.</div>
                <div style={{ fontSize: 12.5, color: "#41506a", lineHeight: 1.5 }}>Mismatched sender, urgency and threats, generic greeting, changed bank details, and a suspicious link — never act on a message like this. Verify through a channel you already trust.</div>
              </div>
            )}
          </div>
        </div>
  ...
  const renderModule = (num: number) => {
    ...
    return (
      <section key={num} id={`module-${num}`} style={{ scrollMarginTop: 24, marginTop: 62, paddingTop: 40, borderTop: "1px solid #e2e7ee" }}>
        ...
        <h2 style={{ fontSize: 30, fontWeight: 700, color: "#16243a", letterSpacing: "-.02em", margin: "12px 0 10px", lineHeight: 1.12 }}>{d.title}</h2>
        <div style={{ color: "#56657a", fontSize: 15.5, lineHeight: 1.55, marginBottom: 16, maxWidth: 720 }}>{d.intro}</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 30 }}>
          {d.points.map((p, pi) => (
            <div key={pi} style={{ display: "flex", gap: 11, alignItems: "center" }}>
              ...
              <div style={{ fontSize: 14, color: "#54627a" }}>{p}</div>
            </div>
          ))}
        </div>
        {rest.map((u, ui) => {
          if (u.type === "content") return (
            <div key={ui} style={{ marginBottom: 32 }}>
              ...
              <h3 style={{ fontSize: 22, fontWeight: 700, color: "#16243a", margin: "7px 0 6px", letterSpacing: "-.01em" }}>{u.title}</h3>
              {u.intro && <div style={{ color: "#56657a", fontSize: 14.5, lineHeight: 1.5, maxWidth: 780, marginBottom: 14 }}>{u.intro}</div>}
  ...
  const renderFinalQ = (idx: number) => {
    ...
    return (
      <div key={idx} style={{ border: "1px solid #e7ebf1", borderRadius: 14, padding: "19px 22px", background: "#fff" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, letterSpacing: ".08em", color: "#9aa6b4" }}>{q.section}</span>
        </div>
        <h3 style={{ fontSize: 18, fontWeight: 700, color: "#16243a", margin: "0 0 13px", lineHeight: 1.32 }}>{q.prompt}</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        ...
        {sub && (
          <div style={{ background: good ? "#eaf5ee" : "#fbece9", borderRadius: 12, padding: "13px 15px", marginTop: 13, borderLeft: `4px solid ${good ? "#1f7a52" : "#b23a3a"}` }}>
            <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: ".12em", fontWeight: 600, color: good ? "#1f7a52" : "#b23a3a", marginBottom: 5 }}>{good ? "CORRECT" : "NOT QUITE"}</div>
            <div style={{ fontSize: 13.5, color: "#3a4759", lineHeight: 1.5 }}>{q.explain}</div>
          </div>
        )}
      </div>
    );
  ...
  const onCopy = () => {
    const text = `To: isms@company-example.com\nSubject: ${emailSubject}\n\n${emailBody}`;
  ...
  const onDownload = () => {
    ...
    lines.push("", "RECOMMENDED FOLLOW-UP ACTIONS", "-".repeat(48), followUp, "", "Prepared for: isms@company-example.com");
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#eef1f5", fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif" }}>
      <aside style={{ position: "fixed", top: 0, left: 0, width: 300, height: "100vh", background: "#16243a", color: "#fff", display: "flex", flexDirection: "column", padding: "24px 20px 18px", zIndex: 10 }}>
        <div style={{ flex: "none" }}>
          <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".16em", color: "#7fa8d6", fontWeight: 600 }}>ISO/IEC 27001 · AWARENESS</div>
      ...
      <main style={{ marginLeft: 300, flex: 1, minWidth: 0 }}>
        <div style={{ maxWidth: 880, margin: "0 auto", padding: "46px 48px 110px" }}>
          ...
          <section id="overview" style={{ scrollMarginTop: 24 }}>
            ...
            <h1 style={{ fontSize: 40, lineHeight: 1.08, fontWeight: 700, color: "#16243a", letterSpacing: "-.02em", margin: "16px 0 0" }}>Security, Privacy &amp; AI Awareness Training</h1>
            <div style={{ fontSize: 17, color: "#56657a", marginTop: 14, lineHeight: 1.5, maxWidth: 640 }}>A short, practical course on protecting information, spotting scams, and using approved AI tools responsibly — for everyone at {companyName}. Work through each module top to bottom; your progress is tracked on the left.</div>
            <div style={{ display: "flex", gap: 26, marginTop: 28, flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}><div style={{ fontSize: 21, fontWeight: 700, color: "#16243a" }}>~30 min</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: ".12em", color: "#8a97a6" }}>DURATION</div></div>
              <div style={{ width: 1, background: "#e2e7ee" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}><div style={{ fontSize: 21, fontWeight: 700, color: "#16243a" }}>6 modules</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: ".12em", color: "#8a97a6" }}>+ ASSESSMENT</div></div>
              <div style={{ width: 1, background: "#e2e7ee" }} />
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}><div style={{ fontSize: 21, fontWeight: 700, color: "#16243a" }}>{passMark}% to pass</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: ".12em", color: "#8a97a6" }}>14 QUESTIONS</div></div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 18, marginTop: 30 }}>
              <button onClick={onBegin} style={cta}>Begin Training</button>
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8a97a6" }}>Mandatory · Your completion is recorded</div>
            </div>
          </section>

          {/* DETAILS */}
          <section id="details" style={{ scrollMarginTop: 24, marginTop: 62, paddingTop: 40, borderTop: "1px solid #e2e7ee" }}>
            ...
            <h2 style={{ fontSize: 27, fontWeight: 700, color: "#16243a", margin: "9px 0 6px" }}>Your details</h2>
            <div style={{ color: "#56657a", fontSize: 14.5, lineHeight: 1.5, marginBottom: 20, maxWidth: 600 }}>These appear on your completion certificate and on the records sent to the ISMS team. Please use your real name and work email.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 520 }}>
              <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, color: "#3a4759" }}>Full name</span>
                <input value={form.name} onChange={e => setForm(s => ({ ...s, name: e.target.value }))} placeholder="e.g. Alex Taylor" style={inputStyle} />
              </label>
              <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, color: "#3a4759" }}>Work email</span>
                <input value={form.email} onChange={e => setForm(s => ({ ...s, email: e.target.value }))} placeholder="e.g. alex.taylor@company-example.com" style={inputStyle} />
              </label>
              <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, color: "#3a4759" }}>Department</span>
          ...
          {/* OBJECTIVES */}
          <section id="objectives" style={{ scrollMarginTop: 24, marginTop: 62, paddingTop: 40, borderTop: "1px solid #e2e7ee" }}>
            ...
            <h2 style={{ fontSize: 27, fontWeight: 700, color: "#16243a", margin: "9px 0 4px" }}>By the end, you'll be able to…</h2>
            <div style={{ color: "#56657a", fontSize: 14.5, lineHeight: 1.5, marginBottom: 20, maxWidth: 720 }}>Nine everyday responsibilities that keep our customers, our people and our business protected.</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "11px 30px" }}>
              {OBJECTIVES.map(o => (
                <div key={o.n} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, fontWeight: 600, color: accent, background: "#eaf0f7", borderRadius: 6, padding: "3px 7px", flex: "none", marginTop: 1 }}>{o.n}</div>
                  <div style={{ fontSize: 14.5, color: "#2f3c4e", lineHeight: 1.4 }}>{o.t}</div>
                </div>
              ))}
            </div>
          ...
          {/* ASSESSMENT */}
          <section id="assessment" style={{ scrollMarginTop: 24, marginTop: 62, paddingTop: 40, borderTop: "1px solid #e2e7ee" }}>
            ...
            <h2 style={{ fontSize: 28, fontWeight: 700, color: "#16243a", margin: "10px 0 6px" }}>Confirm your understanding</h2>
            <div style={{ color: "#56657a", fontSize: 14.5, lineHeight: 1.55, marginBottom: 18, maxWidth: 720 }}>Fourteen short scenarios. Choose the single best answer for each — you can change answers freely until you submit. Feedback and your result appear once you submit.</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 8 }}>
              <div style={{ border: "1px solid #e4e9ef", borderRadius: 10, padding: "12px 14px" }}><div style={{ fontSize: 19, fontWeight: 700, color: "#16243a" }}>14</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".1em", color: "#8a97a6", marginTop: 2 }}>QUESTIONS</div></div>
              <div style={{ border: "1px solid #e4e9ef", borderRadius: 10, padding: "12px 14px" }}><div style={{ fontSize: 19, fontWeight: 700, color: "#16243a" }}>{passMark}%</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".1em", color: "#8a97a6", marginTop: 2 }}>TO PASS</div></div>
              <div style={{ border: "1px solid #e4e9ef", borderRadius: 10, padding: "12px 14px" }}><div style={{ fontSize: 19, fontWeight: 700, color: "#16243a" }}>10–15 min</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".1em", color: "#8a97a6", marginTop: 2 }}>TYPICAL</div></div>
              <div style={{ border: "1px solid #e4e9ef", borderRadius: 10, padding: "12px 14px" }}><div style={{ fontSize: 19, fontWeight: 700, color: "#16243a" }}>{answeredCount} / 14</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".1em", color: "#8a97a6", marginTop: 2 }}>ANSWERED</div></div>
            </div>
            ...
            {!submitted && (
              <div style={{ display: "flex", alignItems: "center", gap: 18, marginTop: 24 }}>
                <button onClick={allAns ? submitAssessment : undefined} style={allAns ? cta : { ...cta, background: "#c4ccd6", cursor: "not-allowed" }}>Submit assessment</button>
                <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11.5, color: "#8a97a6" }}>{allAns ? "All questions answered — ready to submit." : `Answer all 14 to submit · ${answeredCount} done`}</div>
              </div>
            )}
          </section>
          ...
          {/* RESULTS */}
          {submitted && (
            <section id="results" style={{ scrollMarginTop: 24, marginTop: 62, paddingTop: 40, borderTop: "1px solid #e2e7ee" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 18, paddingBottom: 18, borderBottom: "1px solid #eef1f5" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <h2 style={{ fontSize: 23, fontWeight: 700, color: "#16243a", margin: "9px 0 3px" }}>{passed ? "Congratulations — you passed" : "You haven't reached the pass mark yet"}</h2>
                  <div style={{ color: "#56657a", fontSize: 14, lineHeight: 1.5 }}>{passed ? "You've demonstrated the required awareness. Your certificate and ISMS record are below." : "Review the areas below, then retry the assessment when you're ready. You've got this."}</div>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 26, paddingTop: 20 }}>
                <div>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: ".12em", color: "#8a97a6", marginBottom: 11 }}>RESULT SUMMARY</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                    {[
                    ...
                    ].map((row, ri) => <div key={ri} style={{ display: "flex", justifyContent: "space-between", gap: 14, fontSize: 13.5, borderBottom: "1px dashed #e7ebf1", paddingBottom: 7 }}><span style={{ color: "#8a97a6" }}>{row.k}</span><span style={{ color: "#16243a", fontWeight: 600, textAlign: "right" }}>{row.v}</span></div>)}
                  </div>
                </div>
                <div>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: ".12em", color: "#8a97a6", marginBottom: 11 }}>{r.incorrect.length === 0 ? "REVIEW" : "AREAS TO REVIEW"}</div>
                  {r.incorrect.length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {r.incorrect.map((mm, mi) => (
                        <div key={mi} style={{ background: "#fbf3f3", borderLeft: "3px solid #b23a3a", borderRadius: "0 8px 8px 0", padding: "10px 13px" }}>
                          <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, letterSpacing: ".1em", color: "#b23a3a", fontWeight: 600 }}>{mm.q.section}</div>
                          <div style={{ fontSize: 12.5, color: "#3a4759", lineHeight: 1.45, marginTop: 3 }}>{mm.q.prompt}</div>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {!passed && (
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 24, paddingTop: 18, borderTop: "1px solid #eef1f5" }}>
                  <button onClick={retry} style={cta}>Review &amp; retry assessment</button>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11.5, color: "#8a97a6" }}>Re-read the flagged areas above, then try again.</div>
                </div>
              )}
              ...
              {passed && (
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "30px 0 14px" }}>
                    <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: ".12em", color: "#8a97a6" }}>CERTIFICATE OF COMPLETION</div>
                    <button onClick={doPrint} style={ghost}>Print / Save as PDF</button>
                  </div>
                  <div id="certificate" style={{ border: "1px solid #e2e7ee", borderRadius: 14, padding: "40px 46px", background: "linear-gradient(180deg,#ffffff 0%,#f8fafc 100%)", position: "relative", overflow: "hidden" }}>
                    <div style={{ position: "absolute", inset: 10, border: "1.5px solid #dbe3ec", borderRadius: 10, pointerEvents: "none" }} />
                    <div style={{ position: "relative", textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 13, color: "#7a8696", marginTop: 24 }}>This certifies that</div>
                      <div style={{ fontSize: 30, fontWeight: 700, color: "#16243a", margin: "8px 0 4px", letterSpacing: "-.01em" }}>{form.name || "[Your name]"}</div>
                      <div style={{ fontSize: 14, color: "#56657a", maxWidth: 520, margin: "14px auto 0", lineHeight: 1.6 }}>successfully completed the <strong style={{ color: "#16243a" }}>Security, Privacy &amp; AI Awareness Training</strong> for {companyName}, meeting the required standard of awareness competence.</div>
                      <div style={{ display: "flex", justifyContent: "center", gap: 42, marginTop: 28, flexWrap: "wrap" }}>
                        <div><div style={{ fontSize: 17, fontWeight: 700, color: "#16243a" }}>{date}</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".12em", color: "#9aa6b4", marginTop: 3 }}>DATE COMPLETED</div></div>
                        <div style={{ width: 1, background: "#e2e7ee" }} />
                        <div><div style={{ fontSize: 17, fontWeight: 700, color: "#1f7a52" }}>{r.pct}% · PASS</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".12em", color: "#9aa6b4", marginTop: 3 }}>ASSESSMENT SCORE</div></div>
                        <div style={{ width: 1, background: "#e2e7ee" }} />
                        <div><div style={{ fontSize: 17, fontWeight: 700, color: "#16243a" }}>{trainingVersion}</div><div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9.5, letterSpacing: ".12em", color: "#9aa6b4", marginTop: 3 }}>TRAINING VERSION</div></div>
                      </div>
                      <div style={{ marginTop: 28, paddingTop: 18, borderTop: "1px solid #eaeef3", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, color: "#9aa6b4" }}>CERTIFICATE ID · {completionId}</div>
                        <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, color: "#9aa6b4" }}>ISO/IEC 27001 · AWARENESS EVIDENCE</div>
                      </div>
                    </div>
                  </div>

                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "30px 0 14px" }}>
                    <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: ".12em", color: "#8a97a6" }}>ISMS RESULTS RECORD</div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                    <div style={{ border: "1px solid #e2e7ee", borderRadius: 12, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                      <div style={{ padding: "11px 16px", background: "#16243a", color: "#fff", display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#3a86c8" }} />
                      ...
                      <div style={{ padding: "15px 18px", fontSize: 13, lineHeight: 1.6 }}>
                        <div style={{ display: "flex", gap: 8 }}><span style={{ color: "#9aa6b4", width: 54, flex: "none" }}>To</span><span style={{ color: "#16243a", fontWeight: 600 }}>isms@company-example.com</span></div>
                        <div style={{ display: "flex", gap: 8 }}><span style={{ color: "#9aa6b4", width: 54, flex: "none" }}>Subject</span><span style={{ color: "#16243a", fontWeight: 600 }}>{emailSubject}</span></div>
                        <div style={{ borderTop: "1px solid #eef1f5", margin: "11px 0" }} />
                        <div style={{ color: "#3a4759", whiteSpace: "pre-wrap", fontSize: 12.5, lineHeight: 1.65 }}>{emailBody}</div>
                      </div>
                    </div>
                    <div style={{ border: "1px solid #e2e7ee", borderRadius: 12, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                      <div style={{ padding: "11px 16px", background: "#f7f9fb", borderBottom: "1px solid #eef1f5" }}>
                        <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, letterSpacing: ".1em", color: "#56657a" }}>STRUCTURED RESULTS RECORD</span>
                      </div>
                      <div style={{ padding: "6px 18px" }}>
                        {reportRows.map((rr, rri) => <div key={rri} style={{ display: "flex", justifyContent: "space-between", gap: 14, fontSize: 12.5, borderBottom: "1px dashed #e7ebf1", padding: "9px 0" }}><span style={{ color: "#8a97a6", flex: "none" }}>{rr.k}</span><span style={{ color: "#16243a", fontWeight: 600, textAlign: "right" }}>{rr.v}</span></div>)}
                      </div>
                    </div>
                  </div>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#9aa6b4", marginTop: 14 }}>Sending happens outside this training — copy or download the record above for your ISMS evidence.</div>
                </div>
              )}
            </section>
          )}
